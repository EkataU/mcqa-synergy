...
def collect_T_hidden_and_logits(
    ...
    for start in tqdm(range(0, len(prompts), batch_size), desc="[Module1] Forward batches"):
        ...
        for layer_idx in range(num_layers):
            ...
            # NEW: predictive entropy and store it
...
def main():
    ...
    # ---------- NEW: per-layer, per-dialect proto accuracy ----------
    print("[Module1] Computing per-layer, per-dialect proto accuracy (P(T))")
    ...
    for l in sorted(layer_scores_T.keys()):
        ...
        for d in sorted(np.unique(dial_arr)):
            ...
            rows_T_dialect.append({
                "layer": l,
                "dialect": d,
                "ProtoAcc_T_dialect": float(acc_d),
            })

    layer_metrics_T_by_dialect = pd.DataFrame(rows_T_dialect).sort_values(["layer","dialect"])
    ...
    for l in sorted(pooled_T.keys()):
        ...
        entropy = float(np.mean(layer_scores_T[l]["entropy"]))

        overall_mean = X2.mean(axis=0)
        langs = np.unique(dialects)
        # NEW: per-dialect centroids and within-cluster variance in PCA space
        ...
        geom_rows_T.append({
                    ...
                    "Entropy_lang_T": entropy,   # now predictive entropy, varies by layer
    ...
    out_metrics_dialect = os.path.join(args.out_dir, "layer_metrics_T_by_dialect.csv")
    out_pca_centroids   = os.path.join(args.out_dir, "pca_centroids_T.csv")

    layer_metrics_T_by_dialect.to_csv(out_metrics_dialect, index=False)
    pd.DataFrame(pca_centroids_rows).to_csv(out_pca_centroids, index=False)

    print(f"[Module1] Saved per-dialect metrics to: {out_metrics_dialect}")
    print(f"[Module1] Saved PCA centroids to     : {out_pca_centroids}")
